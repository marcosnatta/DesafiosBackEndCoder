...
class CartService {
  ...
  async addProductToCart(cartId, productId, quantity) {
    try {
      const cart = await this.cartsMongo.addProductToCart(
        cartId,
        productId,
        quantity
      );
  ...
  async removeProductFromCart(cartId, productId) {
    try {
      const cart = await this.cartsMongo.removeProductFromCart(
        cartId,
        productId
      );
  ...
  async updateCart(cartId, updates) {
    try {
      const cart = await this.cartsMongo.updateCart(cartId, updates);
      console.log(cartId, updates)
  ...
  async updateProductQuantity(cartId, productId, newQuantity) {
    try {
      const cart = await this.cartsMongo.updateProductQuantity(
        cartId,
        productId,
        newQuantity
      );
